...
public class GitLabPlugin implements Plugin {
    ...
    // Add slack webhook
    public static final String GITLAB_SLACK_USERNAME = "sonar.gitlab.slack.username";
    public static final String GITLAB_SLACK_HOOK = "sonar.gitlab.slack.hook";
    public static final String GITLAB_SLACK_ROOM = "sonar.gitlab.slack.room";
    ...
    public static List<PropertyDefinition> definitions() {
        return Arrays
                .asList(PropertyDefinition.builder(GITLAB_URL).name("GitLab url").description("URL to access GitLab.").category(CATEGORY).subCategory(SUBCATEGORY).defaultValue("https://gitlab.com")
                        ...
                        PropertyDefinition.builder(GITLAB_SLACK_HOOK).name("Slack webhook url").description("This is used for sending slack message")
                                .category(CATEGORY).subCategory(SUBCATEGORY).type(PropertyType.STRING)
                                .index(33).build(),
                        PropertyDefinition.builder(GITLAB_SLACK_ROOM).name("Slack channel name").description("Which room the message sent to")
                                .category(CATEGORY).subCategory(SUBCATEGORY).type(PropertyType.STRING)
                                .index(34).build(),
                        PropertyDefinition.builder(GITLAB_SLACK_USERNAME).name("Slack user name").description("Name display in slack channel")
                                .category(CATEGORY).subCategory(SUBCATEGORY).type(PropertyType.STRING)
                                .index(35).build()
